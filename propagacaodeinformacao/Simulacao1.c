...
#define Prob_Inf 0.50 //Probabilidade do individuo suscetivel se infectar
#define Inf_0 1		  //Quantidade inicial de infectados
#define Ite 365		  //Quantidade de iterações da simulação
#define TipoGrafo 2	  //Tipo de Grafo (Reg=0, Rand=1, Livre de escala=2, Mod= 3)
#define Grau 2		  //Grau medio/2
...
void PrintaVertices(FILE *graph, TABB *Infectados);
...
int main()
{
	int n, no, j, i, k, r, boo, **mat_graph, tam_inf = 0, tam_inf_poss = 0, vet_inf[MAX], vet_poss[MAX];
	pNo pont;
	...
	FILE *pont_arq;
	pont_arq = fopen("Simulacao1.txt", "wt");
	...
	for (i = 0; i < Ite; i++)
	{
		...
		//Printa a iteração, quantidade de infectados e os nós
		//printf("\nNumero de Infectados %d - Iteracao %d:",tam_inf, i+1); EmOrdem(Infectados->Raiz); printf("\n");
		...
		for (j = 0; j < tam_inf; j++)
		{
			...
			Elementos(ListaAdj[vet_inf[j]]->Raiz, &tam_inf_poss, vet_poss);

			//Verifica todos os nós ligados ao nó infectado j
			for (k = 0; k < tam_inf_poss; k++)
			{
				no = vet_poss[k];
				pont = Pesquisa(Infectados->Raiz, no);

				//Se pont for NULL, entao o no nao esta na lista de infectados, checamos se esse no vai ser infectado
				if (pont == NULL)
				{
					//Escolhe um numero aleatorio de 0 a 100
					r = EscolheProb(100);
					//Se o teste VerificaProb retornar 1, entao o nó foi infectado
					if (VerificaProb(Prob_Inf, r))
						Insere(&(Infectados->Raiz), no);
				}
			}
		}
	}
	PrintaVertices(graph_f, Infectados);
