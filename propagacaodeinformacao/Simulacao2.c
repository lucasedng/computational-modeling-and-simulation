...
#define Prob_Inf 0.80 //Probabilidade do individuo suscetivel se infectar
#define Prob_Rec 0.10 //Probabilidade do individuo infectado se recuperar
#define Inf_0 250	  //Quantidade inicial de infectados
#define Ite 100		  //Quantidade de iterações da simulação
...
int main()
{
	int n, no, j, i, k, r, **mat_graph, tam_inf = 0, tam_poss = 0, vet_inf[MAX], vet_poss[MAX], vet_rec[MAX], tam_rec = 0;
	...
	FILE *pont_arq;
	pont_arq = fopen("Simulacao2.txt", "wt");
	...
	for (i = 0; i < Ite; i++)
	{
		...
		for (j = 0; j < tam_inf; j++)
		{
			...
			{
				//Remove o nó da lista de infectados.
				Remove(&(Infectados->Raiz), no);
				//Insere o nó na lista de infectados (c/imunidade).
		...
		for (j = 0; j < tam_inf; j++)
		{
			...
			for (k = 0; k < tam_poss; k++)
			{
				...
				//Se o nó nao estiver na lista de infectados ou recuperados, fazemos o teste para verificar se ele se infectou
				if ((pont_inf == NULL) && (pont_rec == NULL))
				{
					r = EscolheProb(100);
