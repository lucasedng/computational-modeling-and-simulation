...
#define Prob_Inf 0.25	 //Probabilidade do individuo suscetivel se infectar
#define Prob_Rec 0.90	 //Probabilidade do individuo infectado se recuperar
#define Prob_InfRec 0.20 //Probabilidade do individuo recuperado se infectar
#define Inf_0 500		 //Quantidade inicial de infectados
#define Ite 365			 //Quantidade de iterações da simulação
#define TipoGrafo 0		 //Tipo de Grafo (Reg=0, Rand=1, Livre de escala=2, Mod= 3)
#define Grau 1			 //Grau medio/2
...
//void PrintaVertices(FILE *graph, TABB* Infectados, TABB* Recuperados);
...
int main()
{
	int n, no, j, i, k, r, **mat_graph, tam_inf = 0, tam_rec = 0, tam_poss = 0, vet_inf[MAX], vet_poss[MAX], vet_rec[MAX], boo;
	pNo pont_inf, pont_rec;
	...
	FILE *pont_arq;
	pont_arq = fopen("Simulacao3.txt", "wt");
	...
	for (i = 0; i < Ite; i++)
	{
		...
		for (j = 0; j < tam_inf; j++)
		{
			...
			for (k = 0; k < tam_poss; k++)
			{
				...
				//Se o nó nao estiver na lista de infectados, fazemos o teste para verificar se ele se infectou
				if (pont_inf == NULL)
				{
					r = EscolheProb(100);

					//Se o nó não estiver na lista de infectados, ele tem uma prob Prob_Inf de se infectar
					if (pont_rec == NULL)
					{
						boo = VerificaProb(Prob_Inf, r);
					}
					//Se o nó estiver na lista de infectados, ele tem uma prob Prob_InfRec de se infectar novamente
					else
					{
